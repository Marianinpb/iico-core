...
===========================
Tests del ShellBridge: ejecución de skills como subprocesos.
"""
...
def make_python_skill(tmp_path, script_content: str, name: str = "test_skill") -> SkillDefinition:
    """Crea una SkillDefinition temporal con un script Python dado."""
...
def test_execute_simple_python(bridge, tmp_path):
    ...
    skill = make_python_skill(
        tmp_path,
        'import json, sys\n'
        'args = json.loads(sys.stdin.read())\n'
        'print(json.dumps({"echo": args.get("value", "")}))\n'
    )
    result = bridge.execute(skill, {"value": "hello"})
...
def test_execute_duration_tracked(bridge, tmp_path):
    ...
    skill = make_python_skill(tmp_path, 'print("ok")\n')
    result = bridge.execute(skill, {})
    assert result.duration_ms >= 0.0


def test_execute_stdout_captured(bridge, tmp_path):
    skill = make_python_skill(tmp_path, 'print("resultado_capturado")\n')
    result = bridge.execute(skill, {})
...
def test_execute_timeout(tmp_path):
    ...
    skill = make_python_skill(tmp_path, 'import time\ntime.sleep(10)\n')
    result = bridge.execute(skill, {})
...
def test_execute_calculator_skill_integration(bridge, tmp_path):
    """Prueba el protocolo JSON stdin/stdout con la skill calculator real."""
    # Script idéntico al de la skill calculator
    calc_script = (
    ...
    skill = make_python_skill(tmp_path, calc_script, name="calculator")
    result = bridge.execute(skill, {"expression": "2 + 2 * 3"})
    assert result.success
    data = json.loads(result.output)
...
def test_tool_result_success_property(bridge, tmp_path):
    skill = make_python_skill(tmp_path, 'import sys\nsys.exit(0)\n')
    result = bridge.execute(skill, {})
    assert result.success is True


def test_tool_result_to_dict(bridge, tmp_path):
    skill = make_python_skill(tmp_path, 'print("x")\n')
    result = bridge.execute(skill, {})
    d = result.to_dict()
    assert "skill" in d
    assert "output" in d
